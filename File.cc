...
void Page :: StartFromBeginning () {
	myRecs->MoveToStart();
}

int Page :: TotalRecordsPresent() {
	return numRecs;
}

int Page :: IsPageEmpty() {
	if(numRecs == 0) {
		return 1;
	}
	else {
...
int Page :: Append (Record *addMe) {
	...
	myRecs->Insert(addMe);
	numRecs++;
	// cout << "new record added: " << numRecs << endl;
	return 1;	
}
...
void Page :: ToBinary (char *bits) {
	...
	((int *) bits)[0] = numRecs;
	// cout << "TO binary - Records Added: " << numRecs << endl;
...
void Page :: FromBinary (char *bits) {
	...
	numRecs = ((int *) bits)[0];
	// cout <<"Forund Thse record: " << numRecs << endl;
	// sanity check
